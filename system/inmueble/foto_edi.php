...
$titulo = "Fotografias de inmueble"; 
$ubicacion = "Inmueble";
if(isset($_GET['id_inmu']))
{
    $inmueble = $_GET['id_inmu'];
    $_SESSION['inmu'] = $_GET['id_inmu'];
    $consulta = mysqli_query($conn,"SELECT * FROM inmueble WHERE id_inmueble = '$inmueble' ");
    ...
    if(isset($_FILES['imagen']))
	{
		...
		if($type_img == "image/jpg" || $type_img == "image/jpeg" || $type_img == "image/png")
		{
			...
			{	
                ...
                if($filas == 0){
                    $destino = $_SERVER['DOCUMENT_ROOT'] . '/yolitaV3/img/inmuebles/'; 
                    move_uploaded_file($_FILES['imagen']['tmp_name'],$destino.$nom_img);
                    $alta = "INSERT INTO imagenes (img, id_inmueble, est_img) VALUES ('$nom_img', '$inmueble', 1)";
                    if(mysqli_query($conn, $alta))
                    {
                        $alert="Imagen Insertada con exito";
                        echo '<META HTTP-EQUIV="REFRESH" CONTENT="2;URL=http://localhost/yolitav3/system/inmueble/foto_edi.php?id_inmu='.$inmueble.'">';
                    }else
                    {
...
<?php include "../sidebar/sidebar.php"; ?> 
    ...
    <h3><?php echo $fila['descripcion']; ?></h3>
        <div class="row">
            <?php
            $sql = mysqli_query($conn,"SELECT * FROM imagenes WHERE id_inmueble = '$inmueble' AND est_img = 1");
            $respuesta = mysqli_num_rows($sql);
            if($respuesta > 0){
                while($data = mysqli_fetch_array($sql)){
                ?>
                <a href="eliminar_img.php?id_img=<?php echo $data['id_img']; ?>">
                    <img src="../../img/inmuebles/<?php echo $data['img'] ?>" onmouseout="this.src='../../img/inmuebles/<?php echo $data['img']; ?>';" onmouseover="this.src='../../img/inmuebles/cover.png';" style="width: 350px; height:350px; margin:20px; border-radius: 10px;" alt="imagen inmueble">
                </a>
            <?php
                }
            }else
...
<div class="modal fade" id="agregar" tabindex="-1" role="dialog" aria-labelledby="basicModal" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="card">
                <form action="foto_edi.php?id_inmu=<?php echo $_SESSION['inmu']; ?>" method="POST" enctype="multipart/form-data">
                    <input type="hidden" name="id_edi"  value = "<?php echo $inmueble; ?>">
                    <div class="card-body">
                        <!-- foto de perfil -->
