...
$sql = mysqli_query($conn, "SELECT u.nom_us, u.app_us, cli.nom_cli, cli.ape_cli, i.descripcion,c.monto_cancelado,c.num_cuota, c.total_cuota,c.fecha_cuota FROM cuota as c, venta as v, cliente as cli, inmueble as i, usuario as u WHERE c.id_cli = cli.id_cli AND v.id_venta = c.id_venta AND v.id_inmueble = i.id_inmueble AND v.id_us = u.id_us AND c.est_cuota = 1");
$numero = 0;
if(isset($_GET['id']))
{
    $id_des1 = $_GET['id'];
...
{
    $id_des3 = $_GET['id2'];
    $delete = "UPDATE cuota SET est_cuota = 0 WHERE id_cuota = $id_des3";
            ...
            <table id="tabla1" class="table table-bordered table-striped">
                ...
                <?php while($vector = mysqli_fetch_array($sql)){ $numero++; ?>
                    <tr>
                        ...
                        <td><a href="ventas_des.php?id2=<?php echo $vector['id_venta']; ?>" class="btn btn-outline-danger btn-sm">Eliminar</a></td>
                    </tr>
                <?php } ?>
                </tbody>
            </table>
