...
$sql = mysqli_query($conn,"SELECT * FROM cargo WHERE est_cargo = '0'");
$numero=0;
...
if(!empty($_GET))
{
    if(empty($_GET['id']))
    {
        header('location: cargo_hab.php');
    }else
    {
        $id_cargo = $_GET['id'];
        //metodo de elminicion logica
        $cmd = "UPDATE cargo SET est_cargo = '1' WHERE id_cargo = '$id_cargo'";
        if(mysqli_query($conn,$cmd))
        {
            $alert = 'Cargo habilitado con exito';
            header('refresh:3; url=cargo_hab.php');
        }else
        {
            ...
            <table id="tabla1" class="table table-bordered table-striped">
                ...
                <?php while($data = mysqli_fetch_array($sql)){ $numero++;?>
                    <tr>
                        ...
                        <td><a class="btn btn-outline-success btn-sm" href="cargo_hab.php?id=<?php if(isset($data['id_cargo'])){ echo $data['id_cargo']; } ?>">Habilitar</a></td>
                    </tr>
                <?php } ?>
                </tbody>
            </table>
