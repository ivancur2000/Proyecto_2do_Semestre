<?php
session_start();
$titulo = "Lista de planes de pago deshabilitados"; 
$ubicacion = "Planes de pago";
require_once '../../resuorces/config/database.php';
//lista
$sql = mysqli_query($conn, "SELECT * FROM plan_pago WHERE est_plan = '0'");
$cont = 0;
//eliminacion de datos
if(!empty($_GET)){
    if(empty($_GET['id_del']))
    {
        header('Location: hab_plan.php');
    }
    $id = $_GET['id_del'];
    ...
    {
        $del ="UPDATE plan_pago SET est_plan='1' WHERE id_pp = '$id'";
        if(mysqli_query($conn, $del))
        {
            $alert="Dato habilitado con exito";
            header('refresh:1;url=hab_plan.php');
        }else
        {
            ...
            <table id="tabla1" class="table table-bordered table-striped">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><a class="btn btn-outline-success btn-sm" href="hab_plan.php?id_del=<?php echo $vector['id_pp']; ?>">Habilitar</a> </td>
                    </tr>
                    <?php } ?>
                </tbody>
            </table>
